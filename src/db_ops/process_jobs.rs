...
use crate::{
    db_ops::{auth::get_user_by_token, PgConnection}, errors::AppError, models::{Job, NewJob, Timer, User}, process_tokens::fetch_token, routes::{OperationsWithJobs, TimerCreateRequest}, schema::{jobs, timers, tokens}
};

pub fn insert_job_for_user_into_db(processed_token: String, job: OperationsWithJobs, conn: &mut PgConnection) -> Result<(), AppError> {  // здесь даём токен, а не пользователя
...
pub fn fetch_all_jobs_for_user(processed_token: String, conn: &mut PgConnection) -> Result<Vec<String>, AppError> { 
    ...
    Ok(vec_jobs.iter().map(|job| job.job.clone()).collect::<Vec<String>>())
}

pub fn delete_job(processed_token: String, job_to_delete: Vec<String>, conn: &mut PgConnection) -> Result<String, AppError> {
...
pub fn update_job(processed_token: String, job_to_update: Vec<String>, conn: &mut PgConnection) -> Result<String, AppError> {
    let subquery = tokens::table.filter(tokens::token.eq(processed_token)).select(tokens::user_id).into_boxed();                             // задача удалить строку (или строки) с конкретными работами/работой, в которой user_id = user_id токена, в один запрос к БД
